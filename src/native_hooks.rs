...
async function main() {
  const provider = arg('--provider') || process.env.HERMIP_PROVIDER || (process.env.CLAWHIP_PROVIDER && (console.warn('hermip: warning: env var CLAWHIP_PROVIDER is deprecated; use HERMIP_PROVIDER instead'), process.env.CLAWHIP_PROVIDER)) || 'unknown';
  ...
  const result = spawnSync('hermip', ['native', 'hook', '--provider', provider], {
  ...
  if (result.error) {
    const detail =
      typeof result.error?.message === 'string' && result.error.message.trim()
        ...
        : String(result.error);
    console.error(`[hermip] failed to launch native hook bridge: ${detail}`);
  ...
  if (typeof result.status === 'number' && result.status !== 0) {
    ...
    console.error(`[hermip] native hook bridge exited with status ${result.status}`);
  ...
  if (result.signal) {
    ...
    console.error(`[hermip] native hook bridge terminated by signal ${result.signal}`);
    process.exit(1);
  }
}
